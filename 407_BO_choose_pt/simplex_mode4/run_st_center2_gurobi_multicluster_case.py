...
def create_model_center3_gurobi() -> pyo.ConcreteModel:
    ...
    # Cluster-dependent objective shift (set per scenario)
    m.cluster_obj_shift = pyo.Param(mutable=True, initialize=0.0)

    m.obj_expr = alpha * x_center + gamma * track + eta * couple - beta * nonconvex_pair + rho * shift + OBJ_SHIFT + m.cluster_obj_shift
    return m
...
# have slightly different optimal objective values (Scheme 1).
_CLUSTER_OBJ_OFFSETS = [0.0, 5.0]
...
def build_models_center3_gurobi(
    ...
    for s in range(nscen):
        ...
        # Assign cluster-dependent objective offset
        cluster_id = s % len(_CLUSTER_BASES) if s > 0 else 0
        m.cluster_obj_shift.set_value(_CLUSTER_OBJ_OFFSETS[cluster_id])
