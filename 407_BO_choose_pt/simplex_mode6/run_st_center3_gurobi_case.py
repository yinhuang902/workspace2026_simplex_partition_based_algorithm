"""
run_st_center3_gurobi_case.py — Simplex runner for a synthetic center-attracting
nonconvex QCQP-style two-stage stochastic program with 3 first-stage variables.

Design goals
------------
- First-stage dimension fixed at 3.
- Most scenario-wise optima stay near the center of the first-stage box.
...
For each scenario s, solve
    ...
    min  alpha * (x1^2 + x2^2 + x3^2)
       + gamma * sum_i (x_i + y_i - xi_i)^2
       + eta   * (y1 + y2 + y3)^2
...
The first-stage quadratic term pulls x toward the center. Scenario variation is
mainly absorbed by recourse variables y. Nonconvexity comes from the negative
pairwise quadratic terms in y, while the model remains a Gurobi-compatible
nonconvex QCQP.
"""
import argparse
...
def create_model_center3_gurobi() -> pyo.ConcreteModel:
    ...
    m.x1 = pyo.Var(bounds=(-2.0, 2.0), initialize=0.0)
    m.x2 = pyo.Var(bounds=(-2.0, 2.0), initialize=0.0)
    m.x3 = pyo.Var(bounds=(-2.0, 2.0), initialize=0.0)

    # Second-stage recourse variables
    m.y1 = pyo.Var(bounds=(-1.2, 1.2), initialize=0.0)
    m.y2 = pyo.Var(bounds=(-1.2, 1.2), initialize=0.0)
    m.y3 = pyo.Var(bounds=(-1.2, 1.2), initialize=0.0)
    ...
    m.shift1 = pyo.Param(mutable=True, initialize=0.10)
    m.shift2 = pyo.Param(mutable=True, initialize=-0.05)
    m.shift3 = pyo.Param(mutable=True, initialize=0.08)

    m.cap_rhs = pyo.Param(mutable=True, initialize=1.20)

    # Objective weights (deterministic across scenarios)
    alpha = 5.0   # center attraction for x
    gamma = 2.8   # scenario tracking term
    eta   = 0.8   # recourse coupling / regularization
    beta  = 3.4   # negative pairwise y coupling => nonconvex quadratic
    rho   = 0.35  # mild scenario tilt on y

    # Linear constraints
    m.c1 = pyo.Constraint(expr=m.y1 + m.y2 + m.y3 <= m.cap_rhs)
    m.c2 = pyo.Constraint(expr=-m.cap_rhs <= m.y1 + m.y2 + m.y3)

    x_terms = m.x1**2 + m.x2**2 + m.x3**2
    track = ((m.x1 + m.y1 - m.xi1)**2 +
             (m.x2 + m.y2 - m.xi2)**2 +
             (m.x3 + m.y3 - m.xi3)**2)
    ...
    m.obj_expr = alpha * x_terms + gamma * track + eta * couple - beta * nonconvex_pair + rho * shift + OBJ_SHIFT
    return m
...
_PARAM_BASE = [
    ...
    ("xi2",    0.00, "sym", 0.30),
    ("xi3",    0.00, "sym", 0.30),
    ("shift1", 0.10, "sym", 0.12),
    ("shift2", -0.05, "sym", 0.12),
    ("shift3", 0.08, "sym", 0.12),
    ("cap_rhs", 1.20, "pos", 0.12),
]
...
def build_models_center3_gurobi(
    ...
    for s in range(nscen):
        ...
        if print_first_k_rhs > 0 and s < print_first_k_rhs:
            vals = " ".join(f"{p}={float(pyo.value(getattr(m, p))):.4f}" for p, _, _, _ in _PARAM_BASE[:max_mods])
...
MODE_PARAMS = {
    "smoke": {
        ...
        "ef_time_ub": 30.0,
        "plot_every": None,
        "plot_output_dir": "results/st_center3_gurobi_smoke/plots",
        "output_csv_path": "results/st_center3_gurobi_smoke/simplex_result.csv",
        "use_fbbt":        True,
        "use_obbt":        True,
        "obbt_tol":        1e-2,
        "max_obbt_rounds": 3,
    ...
    "full": {
        ...
        "ef_time_ub": 60,
        "plot_every": None,
        "plot_output_dir": "results/st_center3_gurobi_full/plots",
        "output_csv_path": "results/st_center3_gurobi_full/simplex_result.csv",
        "use_fbbt":        True,
        "use_obbt":        False,
        "obbt_tol":        1e-2,
        "max_obbt_rounds": 3,
...
def main():
    ...
    print("=" * 60)
    print("st_center3_gurobi (Python) — synthetic center-attracting QCQP benchmark")
