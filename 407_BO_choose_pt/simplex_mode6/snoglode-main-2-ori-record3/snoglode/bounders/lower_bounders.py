...
class DropNonants(AbstractLowerBounder):
    ...
    def solve_a_subproblem(self, 
        ...
        # if the solution is optimal, return dual bound (valid LB for minimization)
        if results.solver.termination_condition==TerminationCondition.optimal and \
            ...
            # Use dual bound (problem.lower_bound) for a valid LB.
            # The primal objective can overshoot when MIPGap > 0.
            dual_bound = getattr(results.problem, 'lower_bound', None)
            if dual_bound is not None and dual_bound != float('-inf'):
                return True, dual_bound
            # Fallback to primal objective if dual bound unavailable
