...
def plot_iteration_plotly(iter_id, nodes, tri, active_mask,
    ...
    if per_tet:
        ...
        # ========== LAYER 4: CS points of highlighted (LB-best) simplex ==========
        _cs_legend_added = False
        for r in per_tet:
            sid = r["simplex_index"]
            if sid not in highlight_simplices:
                continue
            cs_pts = r.get("c_point_per_scene", [])
            cs_x, cs_y, cs_z, cs_hover = [], [], [], []
            for s_idx, pt in enumerate(cs_pts):
                if pt is None:
                    continue
                arr = np.asarray(pt, float)
                if arr.shape == (3,) and np.all(np.isfinite(arr)):
                    cs_x.append(float(arr[0]))
                    cs_y.append(float(arr[1]))
                    cs_z.append(float(arr[2]))
                    cs_hover.append(f"cs[scen {s_idx}]<br>simp={sid}")
            if cs_x:
                fig.add_trace(go.Scatter3d(
                    x=cs_x, y=cs_y, z=cs_z,
                    mode='markers',
                    marker=dict(size=5, color="green", opacity=0.85),
                    name="c_s points (LB simp)",
                    showlegend=(not _cs_legend_added),
                    hoverinfo="text",
                    hovertext=cs_hover,
                ))
                _cs_legend_added = True
