...
from mode6_split import mode6_select_split_point, _DEFAULTS as _MODE6_DEFAULTS
...
# --- old experimental values (tend to choose c_s point) ---
# TOL_MS_C = 1e3
# TOL_C_VERTS = 1e-6

# second one (active) — tend to choose ms point
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    # File: Mode 6 cluster-aware split diagnostic log
    dbg_mode6_path = os.path.join(debug_log_dir, "debug_mode6.txt")
    with open(dbg_mode6_path, "w", encoding="utf-8") as f:
        f.write("# debug_mode6.txt — mode6 cluster-aware split diagnostics\n")
        f.write("# Per-iteration: search_mode, fallback_used, cluster info, scores, chosen point\n\n")
    ...
    while len(nodes) < target_nodes:
        ...
        if not _deferred_first_split:
            ...
            for _mat_round in range(_max_mat):
                ...
                if lb_simp_rec.get("ms_status", "full") != "full":
                    ...
                    # INVARIANT: the LB simplex must be selected from the active set.
                    # Rebuild active mask for the post-split mesh using the
        ...
        with open(dbg_candidates_path, "a", encoding="utf-8") as _dbf:
            ...
            _dbf.write("\n")
        # --- POINT_SELECT_MODE: 1=ms-first, 4=avg cs (LB-best), 6=cluster-aware, 7=face-cluster ---
        POINT_SELECT_MODE = split_mode
            ...
            if lb_simp_rec is None:
                ...
                for _m4_s, _m4_pt in enumerate(_m4_cs_pts_raw):
                    if _m4_pt is not None:
                        _m4_arr = np.asarray(_m4_pt, float)
                        if np.all(np.isfinite(_m4_arr)):
                            _m4_valid_pts.append(_m4_arr)
        ...
        # ---------- Mode 6: cluster-aware split-point selection ----------
        if POINT_SELECT_MODE == 6 and (not stop_due_to_collision):
            _m6_fallback_used = None  # track which fallback was used (if any)
            _m6_search_mode = "interior"  # will be overwritten
            _m6_reason = None  # termination reason (set only if all fallbacks fail)

            if lb_simp_rec is None:
                _m6_reason = "mode6_no_lb_simplex"
            else:
                # Step 1: try the primary mode6 cluster-aware selection
                _m6_result = mode6_select_split_point(
                    lb_simp_rec=lb_simp_rec,
                    all_nodes=nodes,
                    min_dist=min_dist,
                    S=S,
                    verbose=verbose,
                )
                _m6_search_mode = _m6_result.get("search_mode", "interior")

                if _m6_result["split_point"] is not None:
                    # --- Primary success: snap to feature + re-check collision ---
                    _m6_pt_raw = _m6_result["split_point"]
                    try:
                        _m6_snapped, _m6_loc_type, _m6_loc_info = _snap_feature(_m6_pt_raw, lb_simp_rec)
                    except Exception as _m6_snap_exc:
                        if verbose:
                            print(f"[mode6] snap_to_feature raised: {_m6_snap_exc}")
                        _m6_snapped, _m6_loc_type, _m6_loc_info = None, None, None

                    if _m6_snapped is not None and _m6_loc_type != "vertex" \
                            and min_dist_to_nodes(_m6_snapped, nodes) >= min_dist:
                        new_node = _m6_snapped
                        chosen_ms = float('nan')
                        chosen_cand = {
                            "simplex_index": lb_simp_rec["simplex_index"],
                            "scene": -1,
                            ...
                            "cand_pt": _m6_snapped,
                            "_rec": lb_simp_rec,
                            "loc_type": _m6_loc_type,
                            "loc_info": _m6_loc_info,
                            "pt_source": "mode6_cluster",
                        }
                        if verbose:
                            print(f"Chosen node (MODE6) {tuple(map(float, _m6_snapped))} "
                                  f"search_mode={_m6_search_mode}, "
                                  f"score={_m6_result['score']:.6f}, "
                                  f"loc={_m6_loc_type} "
                                  f"(simp T{lb_simp_rec['simplex_index']})")
                    else:
                        # Snap failed, snapped to vertex, or post-snap collision
                        if verbose:
                            _reason = "snap_failed" if _m6_snapped is None \
                                else "vertex" if _m6_loc_type == "vertex" \
                                else "post_snap_collision"
                            print(f"[mode6] Primary point rejected after snap: {_reason}")
                        # Fall through to fallback chain below (split_point treated as None)
                        _m6_result = dict(_m6_result, split_point=None,
                                          reason="post_snap_rejection")
                # --- Fallback chain: only when primary did not produce new_node ---
                if new_node is None and _m6_result.get("split_point") is None:
                    _m6_primary_reason = _m6_result.get("reason", "unknown")
                    if verbose:
                        print(f"[mode6] Primary selection failed: {_m6_primary_reason}")
                        print(f"[mode6] Entering fallback chain...")

                    # Fallback A: centroid-biased avg_cs
                    _m6_centroid_bias_frac = _MODE6_DEFAULTS["centroid_bias_frac"]
                    _m6_cs_pts_raw = lb_simp_rec.get("c_point_per_scene", [])
                    _m6_valid_pts = []
                    for _m6_s, _m6_pt_raw in enumerate(_m6_cs_pts_raw):
                        if _m6_pt_raw is not None:
                            _m6_arr = np.asarray(_m6_pt_raw, float)
                            if np.all(np.isfinite(_m6_arr)):
                                _m6_valid_pts.append(_m6_arr)

                    if _m6_valid_pts:
                        _m6_avg_cs = np.mean(_m6_valid_pts, axis=0)
                        _m6_centroid = np.mean(np.asarray(lb_simp_rec["verts"], float), axis=0)
                        _m6_biased = _m6_avg_cs + _m6_centroid_bias_frac * (_m6_centroid - _m6_avg_cs)

                        if np.all(np.isfinite(_m6_biased)) and min_dist_to_nodes(_m6_biased, nodes) >= min_dist:
                            _m6_fallback_used = "centroid_bias"
                            try:
                                cand_pt_pert, loc_type, loc_info = _snap_feature(_m6_biased, lb_simp_rec)
                                if loc_type != "vertex" and min_dist_to_nodes(cand_pt_pert, nodes) >= min_dist:
                                    ...
                                    chosen_cand = {
                                        ...
                                        "loc_type": loc_type,
                                        "loc_info": loc_info,
                                        "pt_source": "mode6_fallback_centroid_bias",
                                    }
                                    if verbose:
                                        print(f"[mode6 fallback] centroid-biased avg_cs: "
                                              f"{tuple(map(float, cand_pt_pert))} (loc={loc_type})")
                                else:
                                    _m6_fallback_used = None  # didn't work, try next
                            except Exception as _m6_fb_exc:
                                if verbose:
                                    print(f"[mode6 fallback] centroid_bias snap failed: {_m6_fb_exc}")
                                _m6_fallback_used = None

                    # Fallback B: plain avg_cs (mode4 style)
                    if new_node is None and _m6_valid_pts:
                        _m6_avg_cs = np.mean(_m6_valid_pts, axis=0)
                        if np.all(np.isfinite(_m6_avg_cs)):
                            try:
                                cand_pt_pert, loc_type, loc_info = _snap_feature(_m6_avg_cs, lb_simp_rec)
                                if loc_type != "vertex" and min_dist_to_nodes(cand_pt_pert, nodes) >= min_dist:
                                    _m6_fallback_used = "mode4_avg_cs"
                                    new_node = cand_pt_pert
                                    chosen_ms = float('nan')
                                    chosen_cand = {
                                        "simplex_index": lb_simp_rec["simplex_index"],
                                        ...
                                        "loc_type": loc_type,
                                        "loc_info": loc_info,
                                        "pt_source": "mode6_fallback_avg_cs",
                                    }
                                    if verbose:
                                        print(f"[mode6 fallback] plain avg_cs: "
                                              f"{tuple(map(float, cand_pt_pert))} (loc={loc_type})")
                            except Exception as _m6_fb_exc2:
                                if verbose:
                                    print(f"[mode6 fallback] avg_cs snap failed: {_m6_fb_exc2}")

                    # Fallback C: longest-edge midpoint
                    if new_node is None:
                        _m6_verts_arr = np.array(lb_simp_rec["verts"], float)
                        _m6_vert_idxs = list(lb_simp_rec["vert_idx"])
                        _m6_edges = []
                        for _ei in range(len(_m6_vert_idxs)):
                            for _ej in range(_ei + 1, len(_m6_vert_idxs)):
                                _midpt = tuple(float(c) for c in 0.5 * (_m6_verts_arr[_ei] + _m6_verts_arr[_ej]))
                                _elen = float(np.linalg.norm(_m6_verts_arr[_ei] - _m6_verts_arr[_ej]))
                                _m6_edges.append((_elen, _ei, _ej, _midpt))
                        _m6_edges.sort(key=lambda x: -x[0])  # longest first

                        for _elen, _ei, _ej, _midpt in _m6_edges:
                            if min_dist_to_nodes(_midpt, nodes) >= min_dist:
                                _m6_fallback_used = "edge_midpoint"
                                new_node = _midpt
                                chosen_ms = float('nan')
                                chosen_cand = {
                                    "simplex_index": lb_simp_rec["simplex_index"],
                                    ...
                                    "_rec": lb_simp_rec,
                                    "loc_type": "edge",
                                    "loc_info": {"edge_verts": (_m6_vert_idxs[_ei], _m6_vert_idxs[_ej])},
                                    "pt_source": "mode6_fallback_edge_midpoint",
                                }
                                if verbose:
                                    print(f"[mode6 fallback] edge midpoint: {_midpt} "
                                          f"(edge v{_m6_vert_idxs[_ei]}-v{_m6_vert_idxs[_ej]}, len={_elen:.3e})")
                                break

                    # All fallbacks exhausted → terminate
                    if new_node is None:
                        _m6_reason = "mode6_all_fallbacks_exhausted"

                # --- Write mode6 diagnostic log ---
                try:
                    with open(dbg_mode6_path, "a", encoding="utf-8") as _m6f:
                        _m6f.write(f"{'='*80}\n")
                        _m6f.write(f"[Iter {it}] Mode 6 Split Diagnostics\n")
                        _m6f.write(f"  simplex_id: T{lb_simp_rec['simplex_index']}\n")
                        _m6f.write(f"  search_mode: {_m6_search_mode}\n")
                        _m6f.write(f"  fallback_used: {_m6_fallback_used}\n")
                        if _m6_result.get("diagnostics"):
                            _diag = _m6_result["diagnostics"]
                            _m6f.write(f"  n_valid_cs: {_diag.get('n_valid_cs', '?')}/{_diag.get('n_total_cs', '?')}\n")
                            _m6f.write(f"  clusters: {_diag.get('cluster_sizes', '?')}\n")
                            _m6f.write(f"  cluster_centers: {_diag.get('cluster_centers', '?')}\n")
                            _disp = _diag.get('dispersion', {})
                            _m6f.write(f"  dispersion: max_pw={_disp.get('max_pairwise_distance', '?')}, "
                                       f"var={_disp.get('variance_trace', '?')}, "
                                       f"spread={_disp.get('cluster_center_spread', '?')}\n")
                            _m6f.write(f"  n_sampled: {_diag.get('n_candidates_sampled', '?')}\n")
                            _m6f.write(f"  n_rejected: {_diag.get('n_rejected', '?')}\n")
                            _m6f.write(f"  n_valid_scored: {_diag.get('n_valid_scored', '?')}\n")
                        if _m6_result.get("split_point") is not None:
                            _m6f.write(f"  chosen_point: {_m6_result['split_point']}\n")
                            _m6f.write(f"  score: {_m6_result['score']:.6f}\n")
                            _bd = _m6_result.get('score_breakdown', {})
                            _m6f.write(f"  score_breakdown: sep={_bd.get('separation', {}).get('score', '?')}, "
                                       f"cen={_bd.get('centrality', {}).get('score', '?')}, "
                                       f"qual={_bd.get('quality', {}).get('score', '?')}, "
                                       f"coll={_bd.get('collision', {}).get('score', '?')}\n")
                            _diag2 = _m6_result.get("diagnostics", {})
                            _m6f.write(f"  nearest_node: #{_diag2.get('nearest_node_idx', '?')} "
                                       f"dist={_diag2.get('nearest_node_dist', '?')} "
                                       f"type={_diag2.get('nearest_node_type', '?')}\n")
                            _c2c = _diag2.get('cluster_to_child', {})
                            for _ck, _cv in _c2c.items():
                                _m6f.write(f"  {_ck}: size={_cv.get('size','?')}, child={_cv.get('child_idx','?')}\n")
                        else:
                            _m6f.write(f"  primary_failure: {_m6_result.get('reason', '?')}\n")
                            if _m6_fallback_used:
                                _m6f.write(f"  fallback_point: {new_node}\n")
                            else:
                                _m6f.write(f"  all fallbacks failed\n")
                        _m6f.write(f"\n")
                        _m6f.flush()
                except Exception as _m6_log_exc:
                    print(f"[mode6] WARNING: failed to write debug log: {_m6_log_exc}")

            # Mode 6 termination: only if ALL fallbacks failed
            if _m6_reason is not None:
                termination_reason = _m6_reason
                print(f"[mode6 STOP] Terminating: {termination_reason} (iter {it})")
                selection_reason_hist.append(_m6_reason)
                _write_early_exit_logs(it, termination_reason, LB_global, UB_global,
                                       lb_rec=lb_simp_rec)
                break
        ...
        if POINT_SELECT_MODE == 7 and (not stop_due_to_collision):
            _m7_reason = None  # set only on terminal failure

            if lb_simp_rec is None:
                _m7_reason = "mode7_no_lb_simplex"
            else:
                # --- Compute box_diag from first-stage variable bounds ---
                ...
                if _m7_n_valid >= 2:
                    ...
                    def _m7_find(x):
                        while _m7_parent[x] != x:
                            _m7_parent[x] = _m7_parent[_m7_parent[x]]
                            x = _m7_parent[x]
                        return x

                    def _m7_union(a, b):
                        ra, rb = _m7_find(a), _m7_find(b)
                        if ra != rb:
                            _m7_parent[ra] = rb
                    ...
                    for _i7 in range(_m7_n_valid):
                        _root = _m7_find(_i7)
                        _m7_clusters.setdefault(_root, []).append(_i7)

                    _m7_cluster_list = list(_m7_clusters.values())
                    _m7_K = len(_m7_cluster_list)

                    # Compute centroids and sizes
                    _m7_centroids = []
                    _m7_sizes = []
                    for _cl in _m7_cluster_list:
                        _pts = np.array([_m7_valid[i] for i in _cl])
                        _m7_centroids.append(_pts.mean(axis=0))
                    ...
                    if _m7_K >= 2:
                        ...
                        _m7_pair = (0, 1)
                        for _i7 in range(_m7_K):
                            for _j7 in range(_i7 + 1, _m7_K):
                                _sc = _m7_sizes[_i7] * _m7_sizes[_j7] * float(
                                    np.linalg.norm(_m7_centroids[_i7] - _m7_centroids[_j7]))
                                if _sc > _m7_best_pair_score:
                                    _m7_best_pair_score = _sc
                                    _m7_pair = (_i7, _j7)

                        _m7_mu1 = _m7_centroids[_m7_pair[0]]
                        _m7_mu2 = _m7_centroids[_m7_pair[1]]
                        ...
                        def _m7_validate_candidate(cand, fa, fb, fc):
                            ...
                            # Node distance
                            dn = min_dist_to_nodes(cand, nodes)
                            if dn < _m7_cand_node_tol:
                                return (False, f"node_dist={dn:.3e}<{_m7_cand_node_tol:.3e}", None)
                            # Vertex distance
                            fv = _m7_verts[np.array([fa, fb, fc])]
                            dv = min(float(np.linalg.norm(cand - fv[k])) for k in range(3))
                            if dv < _m7_cand_vert_tol:
                                return (False, f"vert_dist={dv:.3e}<{_m7_cand_vert_tol:.3e}", None)
                            ...
                            det_G = G[0, 0] * G[1, 1] - G[0, 1] * G[1, 0]
                            if abs(det_G) < 1e-20:
                                return (False, "degen_face_gram", None)
                            r = cand - v0
                            b = np.array([r @ e1, r @ e2])
                            uv = np.array([(G[1, 1] * b[0] - G[0, 1] * b[1]) / det_G,
                                           (-G[1, 0] * b[0] + G[0, 0] * b[1]) / det_G])
                            ...
                            if min_b < _M7_BARY_MIN:
                                return (False, f"min_bary={min_b:.4f}<{_M7_BARY_MIN}", bary)
                            # Child-volume check (mandatory)
                            min_child_vol = min_b * _m7_parent_vol
                            if min_child_vol < _m7_vol_floor:
                            ...
                            for _cname, _cpt in _candidates:
                                ...
                                if _ok:
                                    ...
                                    chosen_cand = {
                                        ...
                                        "scene": -1,
                                        "cand_ms": float('nan'),
                                        "cand_pt": _m7_chosen_pt,
                                        "_rec": lb_simp_rec,
                                        "loc_type": "face",
                                        "loc_info": {"face_verts": _face_verts_global},
                                        "pt_source": f"mode7_{_cname}",
                                    }
                                    _m7_did_cluster_split = True
                                    if verbose:
            ...
            if _m7_reason is not None:
                termination_reason = _m7_reason
                print(f"[mode7 STOP] Terminating: {termination_reason} (iter {it})")
        ...
        if verbose and chosen_cand is not None:
            _is_mode2or3 = chosen_cand.get("pt_source", "").startswith(("mode2", "mode3"))
            _is_mode4 = chosen_cand.get("pt_source", "").startswith("mode4")
            _has_scene = (chosen_cand.get("scene", -1) >= 0)
            if _is_mode2or3 or _is_mode4 or _has_scene:
                ...
                if _is_mode4:
                    # Mode 4: avg_cs — print summary without As/ms/As+ms (not applicable)
                    print(f"  Simplex T{sid}, Type={pt_type}, Point={coords_str}")
                    _m4_loc = chosen_cand.get("loc_type", "N/A")
                    print(f"  loc_type={_m4_loc}")
                    # Show per-scenario Q values at the new point
                    print(f"  Per-scenario Q at new point:")
                    for _s in range(S):
                        print(f"    scene {_s}: Q={float(new_vals[_s]):.4e}")
                    print()
                elif _is_mode2or3:
        ...
        if chosen_cand is not None:
            ...
            except Exception as _subdiv_exc:
                if POINT_SELECT_MODE in (4, 6, 7):
                    _mode_tag = f"mode{POINT_SELECT_MODE}"
                    termination_reason = f"{_mode_tag}_subdivide_failed"
                    print(f"[{_mode_tag} STOP] Terminating: {termination_reason} "
        ...
        for new_idx in new_simplex_indices:
            ...
            # (a3) individual per-scenario c_s points as UB candidates
            cpts_list_indiv = rec.get("c_point_per_scene", [])
            for s_cs, cs_pt_raw in enumerate(cpts_list_indiv):
                if cs_pt_raw is None:
                    continue
                if not all(math.isfinite(v) for v in cs_pt_raw):
                    continue
                cs_pt_indiv = tuple(float(v) for v in cs_pt_raw)

                # Optional dedup: skip if very close to avgMS or avgCS already tested
                _skip = False
                if valid_xms:
                    if np.linalg.norm(np.array(cs_pt_indiv) - np.array(avg_ms_pt)) < 1e-10:
                        _skip = True
                if valid_cpts and not _skip:
                    if np.linalg.norm(np.array(cs_pt_indiv) - np.array(avg_cs_pt)) < 1e-10:
                        _skip = True
                if _skip:
                    continue
                ...
                cs_indiv_val = 0.0
                for s in range(S):
                    cs_indiv_val += evaluate_Q_at(base_bundles[s], first_vars_list[s], cs_pt_indiv)

                _cand_type = f"csScene_s{s_cs}"
                if cs_indiv_val < LB_global_end - 1e-8:
                    print(f"[Iter {it}] WARNING: UB candidate ({_cand_type}) {cs_indiv_val/S:.6e} < LB {LB_global_end/S:.6e} (potential invalid LB)")
                if cs_indiv_val < UB_global_end:
                    _cs_i_mesh = _find_any_simplex_containing_point(cs_pt_indiv, per_tet_end)
                    with open(dbg_ub_mesh_path, "a", encoding="utf-8") as _f:
                        _f.write(f"[Iter {it}] UB CANDIDATE: {_cand_type}_new_simplex\n")
                        _f.write(f"  point       = {_fmt_point(cs_pt_indiv, 6)}\n")
                        _f.write(f"  obj_value   = {cs_indiv_val:.9e} (per-scen: {cs_indiv_val/S:.9f})\n")
                        _f.write(f"  inside_mesh = {_cs_i_mesh['found']}\n")
                        _f.write(f"  simplex_idx = {_cs_i_mesh['simplex_index']}\n")
                        _f.write(f"  tol_used    = {_cs_i_mesh['tol_used']}\n")
                        _f.write(f"  n_checked   = {_cs_i_mesh['n_checked']}\n\n")
                    UB_global_end = cs_indiv_val
                    UB_node = cs_pt_indiv
                    ub_updated_end = True
                    ub_source_end = f"{_cand_type}_new_simplex"
                    ub_simplex_id_this_iter = tuple(sorted(rec["vert_idx"]))
                    ub_candidate_library.append((cs_pt_indiv, cs_indiv_val))
                    if verbose:
                        print(f"[Iter {it}] UB candidate from new simplex {new_idx} ({_cand_type}): {cs_indiv_val/S:.6e} (per scenario)")
                _ub_cand_log.append((new_idx, _cand_type, cs_pt_indiv, cs_indiv_val, cs_indiv_val < UB_global_end + 1e-12))
        ...
        try:
            ...
            try:
                _sel_src = chosen_cand.get("pt_source", "N/A") if chosen_cand is not None else "N/A"
                _sel_rank = _selection_rank if '_selection_rank' in dir() and _selection_rank is not None else "N/A"
        ...
        try:
            ...
            _mscs_lines.append(f"  UB candidates (avgMS/avgCS/csScene points from new children):")
            if _ub_cand_log:
