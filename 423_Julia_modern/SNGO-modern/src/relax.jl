...
function relax(P::ModelWrapper, pr::PreprocessResult, U::Float64=1e10,
    ...
    # MUST occur first so that ub_con_idx == length(P.linconstr) + 1 in updaterelax
    ub_aff = copy(m.obj.aff)
    push!(m.linconstr, LinearConstraintData(
        ub_aff, -Inf, U - m.obj.aff.constant))
    ...
    if isa(qbcId, Dict)
        for (key, value) in qbcId
            ...
            # CRITICAL FIX: The cid generated in preprocessex! was a placeholder.
            # We must assign the actual cid here before adding the McCormick envelopes,
            # so that updaterelax modifies the correct constraints instead of overwriting the UB constraint.
            cid = length(m.linconstr) + 1
            qbcId[(xid, yid)] = (bid, cid)
            if xid != yid
                qbcId[(yid, xid)] = (bid, cid)
            end
