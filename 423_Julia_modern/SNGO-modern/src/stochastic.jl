...
function RandomStochasticModel(createModel::Function, nscen::Int=100;
    ...
    # Store stochastic parameters for logging
    master.ext[:stochastic_params] = Dict(
        "NS" => nscen, "nfirst" => nfirst, "nparam" => nparam,
        "rdl" => rdl, "rdu" => rdu, "adl" => adl, "adu" => adu
    )
...
function copyStoModel(P::ModelWrapper)
    ...
    # Copy stochastic params for logging if they exist
    if haskey(P.ext, :stochastic_params)
        m.ext[:stochastic_params] = copy(P.ext[:stochastic_params])
    end
