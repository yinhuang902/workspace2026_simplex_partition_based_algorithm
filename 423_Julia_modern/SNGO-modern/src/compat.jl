...
function build_jump_model!(mw::ModelWrapper, optimizer_factory)
    ...
    for lc in mw.linconstr
        ...
        c_val = lc.terms.constant
        eff_lb = lc.lb - c_val
        eff_ub = lc.ub - c_val

        if lc.lb == lc.ub
            @constraint(model, expr == eff_lb)
        elseif isfinite(lc.lb) && isfinite(lc.ub)
            @constraint(model, eff_lb <= expr <= eff_ub)
        elseif isfinite(lc.lb)
            @constraint(model, expr >= eff_lb)
        elseif isfinite(lc.ub)
            @constraint(model, expr <= eff_ub)
        end
    end
...
function solve_model!(mw::ModelWrapper, optimizer_factory)
    ...
    elseif status_sym == :Infeasible && ENABLE_DEBUG_IIS
        println("\n      --> [DEBUG-INFEASIBLE] Model Infeasible. Computing IIS directly...")
        try
            JuMP.compute_conflict!(model)
            println("          Conflict computed. Infeasible constraints:")
            for c in JuMP.all_constraints(model, include_variable_in_set_constraints=false)
                status_c = MOI.get(JuMP.backend(model), MOI.ConstraintConflictStatus(), JuMP.index(c))
                if status_c == MOI.IN_CONFLICT
                    println("             ", c)
                end
            end
            println("          Variable bounds in conflict:")
            for v in JuMP.all_variables(model)
                status_v = MOI.get(JuMP.backend(model), MOI.ConstraintConflictStatus(), JuMP.index(v))
                if status_v == MOI.IN_CONFLICT
                    lb_v = JuMP.has_lower_bound(v) ? JuMP.lower_bound(v) : -Inf
                    ub_v = JuMP.has_upper_bound(v) ? JuMP.upper_bound(v) : Inf
                    println("             ", v, " bounds: [", lb_v, ", ", ub_v, "]")
                end
            end
        catch e
            println("          IIS extraction failed: ", e)
        end
    end
