    ...
    for varId in varsInCon
        ...
        if ((P.colCat[varId] == :Bin || P.colCat[varId] == :Int) &&
           ...
           (P.colCat[varId] == :Cont && (xl_trial - xl) >= machine_error)
            ...
            println("      --> [DEBUG-TRAP] ALL-VAR: AffineBackward! assigned varId=", varId, " lower bound: ", xl_trial)
        end
        if ((P.colCat[varId] == :Bin || P.colCat[varId] == :Int) &&
            (xu - xu_trial) >= small_bound_improve) ||
           (P.colCat[varId] == :Cont && (xu - xu_trial) >= machine_error)
            P.colUpper[varId] = xu_trial
            println("      --> [DEBUG-TRAP] ALL-VAR: AffineBackward! assigned varId=", varId, " upper bound: ", xu_trial)
        end
    end
...
function fast_feasibility_reduction_inner!(P::ModelWrapper, pr, U::Float64=1e20)
    ...
    if pr !== nothing
        ...
        for ev in pr.expVariable_list
            ...
            if op == :(<=) || op == :(==)
                if (yl - xmax) >= machine_error
                    println("      --> [DEBUG-FBBT-FAIL] Exp cut op <= killed by yl - xmax >= machine_error")
                    println("          var lvarId=", lvarId, " yl=", yl, " xmax=", xmax)
                    return false
                end
            ...
            if op == :(>=) || op == :(==)
                if (xmin - yu) >= machine_error
                    println("      --> [DEBUG-FBBT-FAIL] Exp cut op >= killed by xmin - yu >= machine_error")
                    println("          var nlvarId=", nlvarId, " xmin=", xmin, " yu=", yu)
                    return false
                end
        ...
        for ev in pr.logVariable_list
            ...
            if op == :(>=) || op == :(==)
                if (xmin - yu) >= machine_error
                    println("      --> [DEBUG-FBBT-FAIL] Log cut op >= killed by xmin - yu")
                    return false
                end
                (xmin - yl) >= small_bound_improve && (yl = xmin - small_bound_improve)
                if (xmax - yu) >= machine_error
            ...
            if xl == 20.0 || xu == 20.0 || yl == 20.0 || yu == 20.0
                println("      --> [DEBUG-TRAP] exp/log set 20.0: idx=", nlvarId, " bounds=[", xl, ", ", xu, "]")
            end
            P.colLower[nlvarId] = xl; P.colUpper[nlvarId] = xu
            P.colLower[lvarId] = yl;  P.colUpper[lvarId] = yu
        end
        ...
        for ev in pr.powerVariable_list
            ...
            if !isinteger(d)
                if b >= 0 && xl <= 0
                    xl = 0.0
                elseif (b >= 0 && xu <= 0) || (b < 0 && xl >= 0)
                    println("      --> [DEBUG-FBBT-FAIL] Power propagation failed sign checks.")
                    return false
                elseif b < 0 && xu >= 0
                    xu = 0.0
                end
            ...
            if op == :(<=) || op == :(==)
                if (yl - xmax) >= machine_error
                    println("      --> [DEBUG-FBBT-FAIL] Power cut op <= failed.")
                    return false
                end
                (yu - xmax) >= small_bound_improve && (yu = xmax + small_bound_improve)
            end
        ...
        if !isempty(P.quadconstr) && !isempty(pr.multiVariable_list)
            for i in 1:length(P.quadconstr)
                ...
                if status == :infeasible
                    println("      --> [DEBUG-FBBT-FAIL] MultiVariable linearBackward! failed for QuadConstraint #", i)
                    return false
                end
    ...
    for (i, lc) in enumerate(P.linconstr)
        status = AffineBackward!(lc.terms, P, lc.lb, lc.ub)
        if status == :infeasible
            println("      --> [DEBUG-FBBT-FAIL] AffineBackward! on LinearConstraint #", i, " returned Infeasible!")
            println("          Bounds were: [", lc.lb, ", ", lc.ub, "]")
            println("          Terms: vars=", lc.terms.vars, " coeffs=", lc.terms.coeffs, " constant=", lc.terms.constant)
            println("      --> [DEBUG-FBBT-FAIL-DUMP] DIRECT P.colLower[1:5]: ", P.colLower[1:5])
            println("      --> [DEBUG-FBBT-FAIL-DUMP] DIRECT P.colUpper[1:5]: ", P.colUpper[1:5])
            for vid in lc.terms.vars
                println("            Var ", vid, " bound: [", P.colLower[vid], ", ", P.colUpper[vid], "]")
            end
            return false
        end
    end
...
function fast_feasibility_reduction!(P::ModelWrapper, pr, U::Float64=1e20)
    ...
    if sum(P.colLower .<= P.colUpper) < n
        for i in 1:n
            if P.colLower[i] > P.colUpper[i]
                println("      --> [DEBUG-FBBT-FAIL] Inconsistent bounds on Var ", i, ". lb=", P.colLower[i], " ub=", P.colUpper[i])
            end
        end
        return false
    end
...
function Sto_fast_feasibility_reduction!(P::ModelWrapper, pr_children,
                                         ...
                                         optimizer_factory::Any = gurobi_simplex_optimizer())
    nfirst = P.numCols
    ...
    while feasibility_reduced >= 1
        ...
        if Rold !== nothing
            ...
            println("      --> [DEBUG-DUMP] Before FBBT, R.colLower[1:5]: ", R.colLower[1:5])
            println("      --> [DEBUG-DUMP] Before FBBT, R.colUpper[1:5]: ", R.colUpper[1:5])
            fb = fast_feasibility_reduction!(R, nothing, UB)
            if !fb
                println("      --> [DEBUG-FBBT] fast_feasibility_reduction!(R) returned false")
                return false
            end
            ...
            if solve_relax && rn == 1
                relaxed_status = solve_model!(R, optimizer_factory)
                ...
                if relaxed_status == :Optimal && ((UB - relaxed_LB) <= mingap || (UB - relaxed_LB)/abs(relaxed_LB) <= mingap)
                    println("      --> [DEBUG-FBBT] R relaxed_status Optimal but gap <= mingap. UB=", UB, " LB=", relaxed_LB)
                    return false
                elseif relaxed_status == :Infeasible
                    println("      --> [DEBUG-FBBT] R relaxed_status is Infeasible")
                    return false
                end
        ...
        for (idx, scenario) in enumerate(scenarios)
            if changed[idx] && idx <= length(pr_children)
                ...
                if !fb
                    println("      --> [DEBUG-FBBT] fast_feasibility_reduction!(scenario ", idx, ") returned false")
                    return false
                end
            end
        end
        ...
        if status == :infeasible
            println("      --> [DEBUG-FBBT] linearBackward! on scenario objective values returned infeasible")
            return false
        end
...
function Sto_medium_feasibility_reduction(P, pr_children, Pex, prex, Rold,
    ...
    println("\n  --> [DEBUG-FBBT] Entering Sto_medium_feasibility_reduction")
    println("      First stage bounds BEFORE fast_feasibility_reduction:")
    for i in 1:minimum([5; P.numCols])
        println("        Var ", i, ": [", P.colLower[i], ", ", P.colUpper[i], "]")
    end

    feasible_fast = Sto_fast_feasibility_reduction!(P, pr_children, Pex, prex, Rold, UB, LB, 0, true, optimizer_factory)
    updateExtensiveBoundsFromSto!(P, Pex)

    println("      First stage bounds AFTER fast_feasibility_reduction. Feasible: ", feasible_fast)
    for i in 1:minimum([5; P.numCols])
        println("        Var ", i, ": [", P.colLower[i], ", ", P.colUpper[i], "]")
    end
    ...
    feasible_opt = true
    if feasible_fast
        feasible_opt = optimality_reduction_range(Pex, prex, Rold, UB, bVarsId, optimizer_factory)
        updateStoBoundsFromExtensive!(Pex, P)
        println("      First stage bounds AFTER optimality_reduction_range. Feasible: ", feasible_opt)
        for i in 1:minimum([5; P.numCols])
            println("        Var ", i, ": [", P.colLower[i], ", ", P.colUpper[i], "]")
        end
    end

    return feasible_fast && feasible_opt
end
...
function Sto_slow_feasibility_reduction(P, pr_children, Pex, prex, Rold,
    ...
    while left_OBBT_inner <= 0.9
        ...
        feasible = Sto_fast_feasibility_reduction!(P, pr_children, Pex, prex, Rold, UB, LB, 0, true, optimizer_factory)
        updateExtensiveBoundsFromSto!(P, Pex)
