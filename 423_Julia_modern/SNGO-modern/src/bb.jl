...
function multi_start!(P::ModelWrapper, nstarts::Int, optimizer_factory)
    ...
    for s in 1:nstarts
        ...
        if status == :Optimal && Pnl.objVal < best_UB
            ...
            best_colVal = copy(Pnl.colVal)
        end
    end

    return (best_UB, best_x, best_colVal)
end
...
function branch_bound(P::ModelWrapper; max_iter::Int=5000)
    ...
    # Variable declarations for finally block scope
    LB = -1e10
    best_UB = 1e10
    gap = Inf
    niter = 0
    solved_nodes = 0
    best_x_first = zeros(Float64, nfirst)
    termination_reason = "unknown"
    final_LB_postprocessed = false

    # Initialize logger
    logger = init_logger(P, scenarios, pr_children)
    ...
    # ── Step 3: Initial bounds (moved before relaxation) ──
    println("\n[3] Computing initial bounds...")
    UB, best_x, best_colVal = multi_start!(P, 3, ipopt_optimizer())
    println("  Initial UB (multi-start): ", UB)
    best_UB = UB
    best_x_first = copy(best_x)
    if !isempty(best_colVal)
        Pex.colVal = copy(best_colVal)
    end

    # ── Step 4: Initial relaxation ──
    println("\n[4] Building initial relaxation...")
    R = relax(Pex, prex, 1e10, Pex.colVal)
    Roriginal = copyModel(R)    # (2.7) keep clean copy for per-node reset
    println("  Relaxation: ", R.numCols, " vars, ", length(R.linconstr), " constrs")
    
    log_model_sizes!(logger, Pex.numCols, length(Pex.linconstr), R.numCols, length(R.linconstr))
    ...
    println("\n[5] Root FBBT...")
    root_feasible = Sto_fast_feasibility_reduction!(P, pr_children, Pex, prex, nothing, UB)
    if !root_feasible
        println("  Root node infeasible!")
        termination_reason = "root_infeasible"
        log_root_info!(logger, UB, -1e10, false, :Infeasible, NaN, :Infeasible, NaN, NaN, NaN, termination_reason)
    ...
    queue = [root]
    niter = 0
    FLB = best_UB    # (2.4a) old code: FLB = UB (old bb.jl:187)
    solved_nodes = 0
    LB = root.LB  # ensure LB is always defined
    ...
    while !isempty(queue) && niter < max_iter
        niter += 1
        solved_nodes += 1
        event_note = "processed"

        # ── Select node (best-bound) — old bb.jl:207-212 ──
        # getGlobalLowerBound: find node with min LB, cap at UB
        best_idx = argmin([n.LB for n in queue])
        node = queue[best_idx]
        deleteat!(queue, best_idx)

        # Global LB = min of remaining nodes' LBs, capped at best_UB
        # (matches old getGlobalLowerBound which does NOT use FLB — line 268 commented out)
        if !isempty(queue)
            LB = min(minimum([n.LB for n in queue]), best_UB)
        else
            LB = min(node.LB, best_UB)
        end
        ...
        gap_display = abs(best_UB - LB) / max(1.0, min(abs(LB), abs(best_UB)))
        @printf("%6d  %14.4f  %14.4f  %10.4f  %6d\n",
                niter, LB, best_UB, gap_display * 100, length(queue))
        ...
        while reduction_relax >= 0.1
            ...
            # Update node.LB and global LB after relax (old bb.jl:427-431)
            node.LB = node_LB
            if !isempty(queue)
                remaining_LB = min(minimum([n.LB for n in queue]), best_UB)
            else
                remaining_LB = best_UB
            end
            LB = min(remaining_LB, node_LB)
        end  # while reduction_relax

        # Fathom if infeasible (old bb.jl:436-439)
        if !feasible || relaxed_status == :Infeasible
            FLB = max(FLB, node_LB)
            event_note = !feasible ? "fathomed_fbbt" : "fathomed_relax"
            if niter == 1
                log_root_info!(logger, best_UB, node_LB, feasible, relaxed_status, relaxed_LB, :NotOptimal, NaN, NaN, NaN, event_note)
            end
            log_iteration!(logger, niter, time() - t_start, LB, best_UB, gap_display * 100, length(queue), 0, 0.0, event_note)
            continue
        end
        ...
        # WS fathoming check (old bb.jl:470-479)
        if WS_status == :Optimal &&
           ((best_UB - WS_LB) <= mingap || (best_UB - WS_LB) <= mingap * min(abs(WS_LB), abs(best_UB)))
            if (best_UB - WS_LB) >= 0 && WS_LB <= FLB
                FLB = WS_LB
            end
            event_note = "fathomed_ws"
            if niter == 1
                log_root_info!(logger, best_UB, node_LB, feasible, relaxed_status, relaxed_LB, WS_status, WS_LB, NaN, NaN, event_note)
            end
            log_iteration!(logger, niter, time() - t_start, LB, best_UB, gap_display * 100, length(queue), 0, 0.0, event_note)
            continue
        end

        # Update node.LB and global LB after WS (old bb.jl:500-504)
        node.LB = node_LB
        if !isempty(queue)
            remaining_LB = min(minimum([n.LB for n in queue]), best_UB)
        else
        ...
        # Prune by bound (old bb.jl:511-513)
        if (best_UB - node_LB) <= mingap ||
           (best_UB - node_LB) <= mingap * min(abs(node_LB), abs(best_UB))
            if (best_UB - node_LB) >= 0 && node_LB <= FLB
                FLB = node_LB
            end
            event_note = "pruned_bounds"
            if niter == 1
                log_root_info!(logger, best_UB, node_LB, feasible, relaxed_status, relaxed_LB, WS_status, WS_LB, NaN, NaN, event_note)
            end
            log_iteration!(logger, niter, time() - t_start, LB, best_UB, gap_display * 100, length(queue), 0, 0.0, event_note)
        ...
        # Upper bound (old bb.jl:516+)
        updateStoBoundsFromSto!(P, PWS_child)
        UB_status, WSfix_status, node_UB, local_UB =
            getUpperBound!(Pex, prex, PWS_child, P, node_LB, WS_status, WSfirst_node, niter)

        WSfix_UB = WSfix_status == :Optimal ? node_UB : NaN
        if niter == 1
            log_root_info!(logger, best_UB, node_LB, feasible, relaxed_status, relaxed_LB, WS_status, WS_LB, local_UB, WSfix_UB, "processed")
        end
        ...
        if node_UB < best_UB
            ...
            # Delete nodes that are now fathomed (old bb.jl:540-553)
            delete_nodes = Int[]
            for (idx, n) in enumerate(queue)
                if (best_UB - n.LB) <= mingap ||
                   (best_UB - n.LB) <= mingap * min(abs(best_UB), abs(n.LB))
                    push!(delete_nodes, idx)
                    if (best_UB - n.LB) >= 0 && n.LB <= FLB
                        FLB = n.LB
                    end
                end
            end
            deleteat!(queue, sort(delete_nodes))
        end

        # Re-check gap after UB update (old bb.jl:570-574)
        if (best_UB - node_LB) <= mingap ||
           (best_UB - node_LB) <= mingap * min(abs(best_UB), abs(node_LB))
            if (best_UB - node_LB) >= 0 && node_LB <= FLB
                FLB = node_LB
            end
            event_note = "pruned_bounds_after_ub"
            # It already logged root info above if niter == 1
            log_iteration!(logger, niter, time() - t_start, LB, best_UB, gap_display * 100, length(queue), 0, 0.0, event_note)
            continue
        end
        ...
        # Update global LB after branching
        if !isempty(queue)
            LB = min(minimum([n.LB for n in queue]), best_UB)
        end
        
        event_note = isempty(queue) ? "branched_and_empty" : "branched"
        log_iteration!(logger, niter, time() - t_start, LB, best_UB, gap_display * 100, length(queue), bVarId, bValue, event_note)
    end
    
    if niter >= max_iter
        termination_reason = "max_iterations"
    elseif isempty(queue)
        termination_reason = "all_nodes_solved"
    end

    # If all nodes were fathomed/solved, set LB to close gap (old bb.jl:616-624)
    if isempty(queue) && niter > 0
        println("  All nodes solved.")
        final_LB_postprocessed = true
        if best_UB > 0
            LB = min(best_UB - mingap, best_UB / (1 + mingap))
        else
            LB = min(best_UB - mingap, best_UB * (1 + mingap))
        end
    end
    ...
    t_elapsed = time() - t_start
        ...
        log_error!(logger, e, catch_backtrace())
        rethrow(e)
    finally
        # Make sure logs are written
        gap = max(0.0, abs(best_UB - LB) / max(1.0, min(abs(LB), abs(best_UB))))
        finish_logger!(logger, LB, best_UB, gap * 100, time() - t_start, solved_nodes, 
                       best_x_first, termination_reason, final_LB_postprocessed)
                       
        # Final console output
        println("\n" * "=" ^ 60)
        println("SNGO Results (Run ID: ", logger.runid, ")")
        println("=" ^ 60)
        @printf("  Lower Bound:    %14.6f (Postprocessed: %s)\n", LB, final_LB_postprocessed)
