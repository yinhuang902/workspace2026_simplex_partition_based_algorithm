...
function preprocess!(mw::ModelWrapper)
    ...
    for i in 1:length(Pcopy.quadconstr)
        ...
        # nonlinearIndexs = indices of vars NOT in any MV group → these STAY in remaining aff
        affcopy.vars = affcopy.vars[nonlinearIndexs]
        affcopy.coeffs = affcopy.coeffs[nonlinearIndexs]
